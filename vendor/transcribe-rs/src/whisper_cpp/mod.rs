...
pub struct WhisperInferenceParams {
    ...
    /// Key = ISO language code (e.g. "ro", "en"), value = prompt text.
    /// When Whisper detects (or is told) the segment language, the matching
    /// entry overrides `initial_prompt` for that segment.
...
impl WhisperEngine {
    ...
    fn infer(
        ...
        if !params.lang_prompts.is_empty() {
            let pairs: Vec<(&str, &str)> = params
                .lang_prompts
                .iter()
                .map(|(k, v)| (k.as_str(), v.as_str()))
                .collect();
            full_params.set_lang_prompts(&pairs);
        }
