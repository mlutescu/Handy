...
extern "C" {
    ...
    struct whisper_full_params {
        ...
        // per-language initial prompts: parallel arrays of language codes and prompt strings
        // when the detected (or specified) language matches lang_prompts_languages[i],
        // lang_prompts_strings[i] is used as the initial_prompt for that segment
        // set n_lang_prompts = 0 and both arrays to NULL to disable
        const char * const * lang_prompts_languages; // e.g. { "ro", "en" }
        const char * const * lang_prompts_strings;   // prompt text for each language
        int                  n_lang_prompts;
