...
pub struct FullParams<'a, 'b> {
    ...
    // storage for per-language prompt raw pointers (heap-allocated CStrings via into_raw)
    lang_prompts_lang_ptrs: Vec<*const c_char>,
    lang_prompts_str_ptrs: Vec<*const c_char>,
}
...
impl<'a, 'b> FullParams<'a, 'b> {
    ...
    pub fn new(sampling_strategy: SamplingStrategy) -> FullParams<'a, 'b> {
        ...
        Self {
            ...
            lang_prompts_lang_ptrs: Vec::new(),
            lang_prompts_str_ptrs: Vec::new(),
        }
    }
    ...
    /// Set per-language initial prompts.
    ///
    /// Each entry is a `(language_code, prompt)` pair, e.g. `("ro", "Bună ziua")`.
    /// After language detection (or with an explicit language), if the segment language
    /// matches a language code in this list, the corresponding prompt is used as the
    /// `initial_prompt` for that segment instead of the global `initial_prompt`.
    ///
    /// Pass an empty slice to clear previously set prompts.
    ///
    /// # Panics
    /// Panics if any language code or prompt string contains a null byte.
    pub fn set_lang_prompts(&mut self, prompts: &[(&str, &str)]) {
        self.lang_prompts_lang_ptrs.clear();
        self.lang_prompts_str_ptrs.clear();

        for (lang, prompt) in prompts {
            self.lang_prompts_lang_ptrs.push(
                CString::new(*lang)
                    .expect("Language code contains null byte")
                    .into_raw() as *const c_char,
            );
            self.lang_prompts_str_ptrs.push(
                CString::new(*prompt)
                    .expect("Prompt string contains null byte")
                    .into_raw() as *const c_char,
            );
        }

        if self.lang_prompts_lang_ptrs.is_empty() {
            self.fp.lang_prompts_languages = std::ptr::null();
            self.fp.lang_prompts_strings   = std::ptr::null();
            self.fp.n_lang_prompts         = 0;
        } else {
            self.fp.lang_prompts_languages = self.lang_prompts_lang_ptrs.as_ptr();
            self.fp.lang_prompts_strings   = self.lang_prompts_str_ptrs.as_ptr();
            self.fp.n_lang_prompts         = prompts.len() as c_int;
        }
    }
